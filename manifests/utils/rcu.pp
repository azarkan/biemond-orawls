...
define orawls::utils::rcu(
  ...
  wls_rcu{ "${rcu_prefix}_${sanitised_title}":
    ...
    statement    => "${oracle_fmw_product_home_dir}/bin/rcu -silent ${action} -databaseType ORACLE -connectString ${rcu_database_url} -dbUser ${rcu_sys_user} -dbRole Normal -honorOMF -schemaPrefix ${rcu_prefix} ${components} -f < ${download_dir}/rcu_passwords_${fmw_product}_${rcu_action}_${rcu_prefix}_${sanitised_title}.txt",
    os_user      => $os_user,
